...
public class SelectionSort {
    public static void main(String[] args) {
        ...
        System.out.println(
            Arrays.toString(getSelectionSortedArray(arr))
        );
    }

    public static int[] getSelectionSortedArray(int[] arr) {
        int smallestNoIndex = 0;

        for(int i=0; i< arr.length - 1; i++){
            smallestNoIndex = i;
            for(int j=i;j<arr.length;j++){
                if(arr[smallestNoIndex] > arr[j]){
                    smallestNoIndex = j;
                }
            }

            if(smallestNoIndex != i){
                //swapping
                int temp = arr[i];
                arr[i] = arr[smallestNoIndex];
                arr[smallestNoIndex] = temp;
            }
        }
